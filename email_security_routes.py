...
def create_email_security_blueprint(id_generator, get_random_color):
    ...
    def email_config(url_addition):
        ...
        transport_status = transport_manager.is_configured()
        config_status = {
            "smtp": bool(transport_status.get("smtp")),
            "imap": bool(transport_status.get("imap")),
            "domain": bool(domain_config.get("configured")),
        }
        ...
        if request.method == "POST":
            ...
            if action == "configure_smtp" or config_type == "smtp":
                smtp_config = {
                ...
                try:
                    if transport_manager.configure_smtp(**smtp_config):
                        ...
                        message = {"type": "error", "text": "SMTP connection failed. Verify server and credentials."}
                except Exception as e:
                    message = {"type": "error", "text": f"SMTP configuration failed: {str(e)}"}
            
            elif action == "configure_imap" or config_type == "imap":
                imap_config = {
                ...
                try:
                    if transport_manager.configure_imap(**imap_config):
                        ...
                        message = {"type": "error", "text": "IMAP connection failed. Verify server and credentials."}
                except Exception as e:
                    message = {"type": "error", "text": f"IMAP configuration failed: {str(e)}"}
            
            elif action == "configure_domain_api" or config_type == "domain":
                try:
            ...
            active_domain = domain_rotation_manager.get_active_domain()
            transport_status = transport_manager.is_configured()
            config_status["smtp"] = bool(transport_status.get("smtp"))
            config_status["imap"] = bool(transport_status.get("imap"))
            config_status["domain"] = bool(domain_config.get("configured"))
