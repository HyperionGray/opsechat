...
def _format_datetime(value, fmt):
    """Format datetime values from either datetime objects or ISO strings."""
...
def rotate_domain(max_price=5.0, auto_confirm=False):
    ...
    if not auto_confirm:
        ...
        print("Auto-confirm enabled; proceeding with purchase.")
...
Examples:
  ...
  python domain_rotation_cli.py list       # List owned domains
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help='Maximum price in USD for search/rotate (default: 5.0)'
    )
    parser.add_argument(
        '--attempts',
        type=int,
        default=5,
        help='Number of search attempts for the search command (default: 5)'
    )
    parser.add_argument(
        '--yes',
        action='store_true',
        help='Skip rotate purchase prompt (automation-friendly)'
    )
    ...
    if args.command == 'config':
        ...
        max_price = max(0.01, args.max_price)
        search_domains(max_price=max_price, attempts=attempts)
    elif args.command == 'rotate':
        max_price = max(0.01, args.max_price)
        rotate_domain(max_price=max_price, auto_confirm=args.yes)
