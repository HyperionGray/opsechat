...
def _deserialize_datetime(value):
    """Deserialize datetime values loaded from JSON."""
...
def _serialize_owned_domains(domains):
    """Convert owned domain records to JSON-safe structures."""
    serialized = []
...
def _deserialize_owned_domains(domains):
    """Restore owned domain records from persisted JSON structures."""
    deserialized = []
    for domain in domains:
        item = dict(domain)
        item['purchased_at'] = _deserialize_datetime(item.get('purchased_at'))
        item['expires_at'] = _deserialize_datetime(item.get('expires_at'))
        deserialized.append(item)
    return deserialized


def _format_datetime(value, fmt):
    """Format datetime-like values without assuming in-memory datetime objects."""
...
def search_domains(max_price: float = 5.0, attempts: int = 5):
    ...
    print(f"\nFound {found} candidate domains in {attempts} attempts.")
    print("\nTo purchase a domain, run: python domain_rotation_cli.py rotate")


def rotate_domain(auto_confirm: bool = False, max_price: float = 5.0, attempts: int = 10):
...
Examples:
  ...
  python domain_rotation_cli.py list       # List owned domains
    ...
    parser.add_argument(
        '--yes',
        action='store_true',
        help='Skip rotation purchase confirmation prompt'
    )
    parser.add_argument(
        '--max-price',
        type=float,
        default=5.0,
        help='Maximum domain purchase/search price in USD (search/rotate)'
    )
    parser.add_argument(
        '--attempts',
        type=int,
        default=10,
        help='Maximum random search attempts (search/rotate)'
    )
    
    args = parser.parse_args()

    if args.max_price <= 0:
        print("Error: --max-price must be greater than 0.")
        sys.exit(2)
    if args.attempts <= 0:
    ...
    if args.command == 'config':
        ...
        rotate_domain(auto_confirm=args.yes, max_price=args.max_price, attempts=args.attempts)
