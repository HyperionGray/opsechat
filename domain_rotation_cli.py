...
def _format_datetime_for_display(value, fmt):
    """Format persisted datetime values safely for CLI output."""
    if hasattr(value, "strftime"):
        return value.strftime(fmt)
    if isinstance(value, str):
        try:
            return DomainRotationManager._parse_datetime(value).strftime(fmt)
        except Exception:
            return value
    return str(value)
...
def get_manager():
    ...
    # Load saved state (with datetime parsing handled by manager)
    manager.load_state(
...
def list_domains():
    ...
    for i, domain in enumerate(domains, 1):
        ...
        print(f"   Purchased: {_format_datetime_for_display(domain['purchased_at'], '%Y-%m-%d %H:%M')}")
        print(f"   Expires: {_format_datetime_for_display(domain['expires_at'], '%Y-%m-%d')}")
        print()
