...
def _format_timestamp(value, format_string):
    """Format persisted datetime values for CLI output."""
...
def list_domains():
    ...
    if removed:
        print(f"Pruned {removed} expired domain(s) from local state.")
...
def prune_domains():
    ...
    if removed:
        print(f"Pruned {removed} expired domain(s).")
    else:
