...
def _coerce_datetime(value):
    """Convert mixed datetime values to a datetime object when possible."""
...
def get_manager():
    ...
    # Load saved state (owned_domains may contain serialized datetimes)
    manager.load_state({
        'monthly_budget': config.get('monthly_budget', 50.0),
...
def list_domains():
    ...
    for i, domain in enumerate(domains, 1):
        ...
        purchased_at = _coerce_datetime(domain.get('purchased_at'))
        expires_at = _coerce_datetime(domain.get('expires_at'))
...
def cleanup_domains():
    """Remove expired local domain records and persist state."""
    manager, config = get_manager()
    removed_count = manager.cleanup_expired_domains()
    save_manager_state(manager, config)
    print(f"\nRemoved {removed_count} expired domain record(s).")
    print(f"Active Domain: {manager.active_domain or 'None'}")
