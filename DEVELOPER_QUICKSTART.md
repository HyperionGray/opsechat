...
- `/health`, `/health/live`, and `/health/ready` contract + security headers
...
The application container healthcheck now probes `/health/ready` by default.
...
- `monitoring.py` - health/readiness payload generation
...
- Keep `/health/live` and `/health/ready` stable and lightweight; deployment checks rely on them.
