...
- `/health` and `/ready` contracts and security headers
...
- `/health` for liveness and lightweight status metadata
- `/ready` for readiness probing by containers/orchestrators

Container and compose healthchecks target `/ready`.
...
- `monitoring.py` - `/health` and `/ready` payload generation
...
- Keep `/health` stable and lightweight for liveness checks.
- Keep `/ready` strict for deployment readiness checks.
