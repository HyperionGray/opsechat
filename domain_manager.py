...
class DomainRotationManager:
    ...
    def configure(self, api_key: str, secret_key: str, monthly_budget: float = 50.0) -> Dict:
        """
        Configure the manager with Porkbun API credentials.
        Returns non-sensitive config metadata for UI/API usage.
        """
    ...
    @staticmethod
    def _mask_secret(secret_value: str, visible_suffix: int = 4) -> str:
        """Mask sensitive values while keeping a short suffix for identification."""
        if not secret_value:
            return ""
        if len(secret_value) <= visible_suffix:
            return "*" * len(secret_value)
        return f"{'*' * (len(secret_value) - visible_suffix)}{secret_value[-visible_suffix:]}"

    @staticmethod
    def _parse_price(price_value: Any, default_value: float = 999.0) -> float:
        """Normalize provider price values to float."""
        if isinstance(price_value, (int, float)):
            return float(price_value)
        if isinstance(price_value, str):
            normalized = price_value.strip().replace("$", "").replace("€", "").replace(",", "")
            try:
    ...
    def _serialize_datetime(value: Any) -> Any:
        """Convert datetime objects to ISO strings for JSON-safe state."""
    ...
    def _deserialize_datetime(value: Any) -> Any:
        ...
        # Convert timezone-aware values to naive UTC datetime for consistency
    ...
    def load_state(self, state: Optional[Dict[str, Any]]):
        """Load state previously generated by export_state()."""
        if not state:
            return
        ...
        # Keep active domain consistent if stale/missing from loaded inventory.
    ...
    def cleanup_expired_domains(self, reference_time: Optional[datetime] = None) -> int:
        """
    ...
    def get_config(self, include_secrets: bool = False) -> Dict[str, Any]:
        """Return manager configuration and budget status metadata."""
        config = {
        ...
        if self.api_client is not None:
            ...
            if include_secrets:
                ...
                config["api_key"] = self._mask_secret(api_key)
                config["secret_key"] = self._mask_secret(api_secret)
