...
# {"status":"healthy","service":"opsechat","version":"0.8.0-alpha","active_rooms":0,"uptime_seconds":12,...}
```

The health payload now includes runtime metadata (`started_at`, `uptime_seconds`, `timestamp`) and current chat endpoint rate-limit settings.
