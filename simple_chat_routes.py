...
def rate_limit_error_response(endpoint: str, retry_after: int):
    """Build a standard rate-limit response with machine-readable metadata."""
    config = RATE_LIMITS.get(endpoint, {})
...
def register_simple_chat_routes(app):
    ...
    def chat_create():
        ...
        if not allowed:
            return rate_limit_error_response("chat_create", retry_after)
    ...
    def simple_chat_messages(room_id):
        ...
        if request.method == 'POST':
            ...
            if not allowed:
                return rate_limit_error_response("chat_message", retry_after)
    ...
    def send_dm():
        ...
        if not allowed:
            return rate_limit_error_response("dm_send", retry_after)
