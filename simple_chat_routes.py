...
def get_rate_limit_status(session_id: str, endpoint: str) -> dict:
    """
    Return detailed rate-limit status for a session/endpoint pair.

    For unknown endpoints, returns a minimal status indicating that no
    explicit in-memory limit is configured.
    """
    config = RATE_LIMITS.get(endpoint)
    if not config:
        return {
    ...
    with _rate_limit_lock:
        endpoints = _rate_limit_store.get(session_id, {})
        recent = [ts for ts in endpoints.get(endpoint, []) if ts > cutoff]
    ...
    if recent:
        oldest = min(recent)
        reset_in = max(int(window - (now - oldest).total_seconds()) + 1, 1)
    else:
        reset_in = 0
...
def _rate_limit_response(session_id: str, endpoint: str, error_message: str, retry_after: int):
    """
    Build a consistent 429 response payload with retry metadata.
    """
    status = get_rate_limit_status(session_id, endpoint)
    response = jsonify({
        "error": error_message,
...
def register_simple_chat_routes(app):
    ...
    def chat_rate_limit_status():
        """Return current per-session rate-limit usage for chat endpoints."""
        if "_id" not in session:
            session["_id"] = generate_secure_dm_id()
            session["username"] = generate_random_username()
            session["color"] = get_random_color_rgb()
        ...
        return jsonify({
            "session_id": sid,
            "limits": {
                endpoint: get_rate_limit_status(sid, endpoint)
