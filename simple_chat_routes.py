...
def cleanup_old_dms():
    ...
    with dm_lock:
        ...
        for dm_id in expired_dms:
            _secure_delete_dm_locked(dm_id)


def _secure_delete_dm_locked(dm_id: str) -> bool:
    """
    Overwrite sensitive DM fields and remove the DM from memory.

    Must be called while holding `dm_lock`.
    Returns True when a DM existed and was removed.
    """
    dm = direct_messages.get(dm_id)
    if not dm:
        return False

    for key in ("message", "room_id", "sender_name", "sender_id"):
        value = dm.get(key)
...
def register_simple_chat_routes(app):
    ...
    def view_dm(dm_id):
        """View a direct message (one-time read: burn after successful view)"""
        with dm_lock:
            ...
            if dm.get("read"):
                _secure_delete_dm_locked(dm_id)
                return jsonify({"error": "DM already viewed"}), 404
            ...
            # One-time semantics: securely burn as soon as it is read.
            _secure_delete_dm_locked(dm_id)
