...
def register_simple_chat_routes(app):
    ...
    chat_create_limit = app.config.get("RATE_LIMIT_CHAT_CREATE", "10 per hour; 3 per minute")
    chat_messages_post_limit = app.config.get("RATE_LIMIT_CHAT_MESSAGES_POST", "60 per minute")
    chat_dm_send_limit = app.config.get("RATE_LIMIT_CHAT_DM_SEND", "20 per hour; 5 per minute")
    ...
    @limiter.limit(chat_messages_post_limit, methods=["POST"])
    ...
    @limiter.limit(chat_dm_send_limit)
    def send_dm():
