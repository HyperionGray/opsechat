...
def _rate_limit_headers(endpoint: str, retry_after: int) -> dict:
    """Build standard rate-limit headers for 429 responses."""
    config = RATE_LIMITS.get(endpoint, {})
    retry_after = max(retry_after, 1)
    return {
        "Retry-After": str(retry_after),
        "X-RateLimit-Limit": str(config.get("max_requests", 0)),
        "X-RateLimit-Remaining": "0",
        "X-RateLimit-Reset": str(retry_after),
        "X-RateLimit-Window": str(config.get("window_seconds", 0)),
    }


def _rate_limited_response(endpoint: str, retry_after: int, message: str):
    """Return a JSON 429 response with rate-limit metadata headers."""
    response = jsonify({"error": message})
    response.status_code = 429
    for header, value in _rate_limit_headers(endpoint, retry_after).items():
