...
def rate_limit_error(message: str, retry_after: int):
    """
    Build a consistent 429 response with retry hints for clients.
    """
    bounded_retry_after = max(int(retry_after), 1)
    response = jsonify({
        "error": message,
    ...
    response.headers["Retry-After"] = str(bounded_retry_after)
