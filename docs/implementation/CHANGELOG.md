...
- HTTP Mail no-JavaScript compose endpoint: `POST /<path>/mail/send`
  with `_address_override` form support for non-JS clients.

### Changed
- Improved repository structure and documentation organization
- Updated CI/CD compliance for better maintainability
- Hardened HTTP Mail mailbox lifecycle to reject writes after mailbox
  destruction (`destroyed` guard in `HttpMailbox.add_message`).

### Fixed
- Completed pending HTTP Mail mailbox-destruction follow-up:
  writes against destroyed mailbox instances now fail safely.
- Added regression tests for:
  - no-JS form compose flow
  - missing-address validation on form compose
  - post-destroy write rejection behavior
