...
- Adaptive in-memory rate-limit backoff for simple chat write endpoints
- Standardized 429 JSON responses with `retry_after` and `Retry-After` header metadata

### Changed
- Improved repository structure and documentation organization
- Updated CI/CD compliance for better maintainability
- Added tests for rate-limit cooldown and backoff escalation behavior
