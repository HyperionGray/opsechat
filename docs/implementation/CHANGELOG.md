...
- HTTP Mail generic compose endpoint: `POST /<path>/mail/send`
- HTTP Mail hardening notes for mailbox-destruction safety and validation

### Changed
- Improved repository structure and documentation organization
- Updated CI/CD compliance for better maintainability
- Mailbox destruction now enforces a destroyed write-guard to prevent stale-reference sends
- Email route session initialization cleaned up to prevent no-session view regressions
