...
- Namecheap domain registrar integration in `domain_manager.py`
- Registrar factory helper for multi-provider domain client selection
- Domain CLI state serialization helpers for reliable restart persistence

### Changed
- Improved repository structure and documentation organization
- Updated CI/CD compliance for better maintainability
- Domain rotation CLI now supports both Porkbun and Namecheap configuration flows
- Domain manager now includes `configure()` and `get_config()` helpers used by email config routes
