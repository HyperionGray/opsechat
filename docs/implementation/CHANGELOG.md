...
- HTTP Mail read-key rotation endpoint/UI (`POST /<path>/mail/<address>/rotate-key`)
- HTTP Mail no-JavaScript compose fallback endpoint (`POST /<path>/mail/send`)
- Dedicated HTTP Mail user guide (`docs/user-guide/HTTP_MAIL.md`)

### Changed
- Improved repository structure and documentation organization
- Updated CI/CD compliance for better maintainability
- HTTP Mail mailbox destruction flow now marks mailboxes as destroyed and refuses late writes
