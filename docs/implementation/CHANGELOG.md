...
- Domain rotation guide refreshed to match implemented manager/CLI/web flow

### Changed
- Improved repository structure and documentation organization
- Updated CI/CD compliance for better maintainability
- Completed domain rotation manager integration for email configuration routes
  (`/email/config` and `/email/domain/rotate`)
- Added JSON-safe domain state import/export helpers to avoid datetime
  serialization failures in CLI persistence
- Added test coverage for domain state serialization and email domain
  configuration/rotation endpoints
