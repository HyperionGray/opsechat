...
- Domain manager configuration/state helpers (`configure`, `get_config`, `load_state`, `export_state`)
- Backward-compatible `rotate-domain.py` wrapper for documented legacy CLI usage

### Changed
- Improved repository structure and documentation organization
- Updated CI/CD compliance for better maintainability
- `domain_rotation_cli.py` now uses manager state import/export for safe datetime persistence
- Domain rotation API route now returns structured success/error details
- Fixed `email_routes.py` undefined session helper in email view path
