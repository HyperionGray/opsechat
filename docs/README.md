...
### Runtime Operations
Endpoints useful for health checks, release validation, and automation.

- `GET /health` - Service health snapshot (`status`, `version`, `active_rooms`, checks)
- `GET /version` - Canonical version endpoint (reads from `VERSION`)
- `GET /metadata` - Release metadata (`name`, `version`, `release_channel`, endpoint map)
