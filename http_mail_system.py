...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> Optional[str]:
        """Add a message; returns message ID, or None if mailbox was destroyed."""
    ...
    def destroy(self) -> None:
        """Securely destroy mailbox contents and block all future operations."""
        with self.lock:
            if self.destroyed:
                return
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then mark the mailbox as destroyed under its own lock, which
          blocks writes through stale references and securely wipes messages.
        """
