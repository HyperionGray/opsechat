...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Once removed from the global mapping, mark destroyed and scrub data.
        # The destroyed flag blocks stale references from accepting new sends.
        with mailbox.lock:
    ...
    def rotate_mailbox_read_key(self, address: str, current_read_key: str) -> Optional[str]:
        """Rotate a mailbox read key after authenticating current read key."""
        with self._lock:
            mailbox = self._mailboxes.get(address)
        if mailbox is None:
            return None
