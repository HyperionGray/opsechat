...
class HttpMailbox:
    ...
    def get_status(self, read_key: str) -> Optional[Dict]:
        """Return metadata for mailbox owner, or None for invalid read_key."""
        if not secrets.compare_digest(read_key, self.read_key):
            return None
        return {
            ...
            "expires_after_hours": MAIL_EXPIRY_HOURS,
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - HttpMailbox exposes a per-mailbox `lock` used by writers.
        - HttpMailbox.add_message refuses writes when `destroyed=True`.
        """
