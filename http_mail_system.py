...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Irreversibly destroy mailbox contents and reject future writes."""
        with self.lock:
            for msg in self.messages:
                msg.overwrite()
            self.messages.clear()
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Now that mailbox is no longer globally reachable, permanently
        # destroy in-memory contents and reject any concurrent late writer.
        mailbox.destroy()
