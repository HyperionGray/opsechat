...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Permanently destroy mailbox message data and block future writes."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def create_mailbox(self) -> HttpMailbox:
        ...
        # Extremely unlikely, but avoid accidental address collisions.
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then destroy mailbox contents under the per-mailbox lock
        ...
        # Now that the mailbox is no longer globally reachable, securely wipe
        # message contents and block any late writes through stale references.
        mailbox.destroy()
