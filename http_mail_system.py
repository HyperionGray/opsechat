...
# Keep mailbox memory bounded under sustained traffic.
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        with self._lock:
            ...
            # Lock mailbox while still holding global storage lock so:
            # 1) no new lookup can obtain this mailbox reference, and
            # 2) concurrent writers must wait until destroyed flag is set.
            with mailbox.lock:
