...
class HttpMailbox:
    ...
    def get_messages(
        ...
        if sender:
            sender_fold = sender.casefold()
            messages = [m for m in messages if m["sender"].casefold() == sender_fold]

        if query:
            query_fold = query.casefold()
            messages = [
                m for m in messages
                if query_fold in m["subject"].casefold() or query_fold in m["body"].casefold()
            ]
