...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> str:
        ...
        Raises:
            RuntimeError: if the mailbox has already been destroyed.
        """
        with self.lock:
            if self.destroyed:
    ...
    def destroy(self) -> None:
        """Overwrite and clear all messages, then permanently disable mailbox writes."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Mailbox is no longer globally reachable; now scrub data and hard-stop
        # any stale references from writing to this mailbox.
        mailbox.destroy()
