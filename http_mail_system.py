...
class HttpMailbox:
    ...
    def get_messages(self, read_key: str) -> Optional[List[Dict]]:
        ...
        with self.lock:
            if self.destroyed:
                return None
            key_matches = secrets.compare_digest(read_key, self.read_key)
        if not key_matches:
            return None
        self._expire_old_messages()
        with self.lock:
            if self.destroyed or not secrets.compare_digest(read_key, self.read_key):
                return None
    ...
    def rotate_read_key(self, current_read_key: str) -> Optional[str]:
        """Rotate and return the new read key after verifying current key."""
        with self.lock:
            if self.destroyed:
                return None
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # mark it destroyed, overwrite, and clear messages under mailbox lock.
        with mailbox.lock:
    ...
    def rotate_mailbox_read_key(self, address: str, read_key: str) -> Optional[str]:
        """Rotate mailbox read key and return new key if authorized."""
        with self._lock:
            mailbox = self._mailboxes.get(address)
        if mailbox is None:
            return None
