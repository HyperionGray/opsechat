...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Securely wipe all messages and block any future writes."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then wipe the detached mailbox in-place to prevent stale references
          from accepting new messages.
        """
        ...
        # Wipe all message content and mark mailbox as destroyed so stale
        # references refuse any future writes.
        mailbox.destroy()
        return True
