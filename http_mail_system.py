...
# Read key entropy (24 bytes -> 32 chars token_urlsafe)
READ_KEY_TOKEN_BYTES = 24
...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> Optional[str]:
        """Add a message; returns the new message ID, or None if mailbox was destroyed."""
    ...
    def rotate_read_key(self, read_key: str) -> Optional[str]:
        """Rotate mailbox read_key after authenticating with the current key."""
        with self.lock:
            if self.destroyed:
                return None
            if not secrets.compare_digest(read_key, self.read_key):
                return None
            self.read_key = _generate_id(READ_KEY_TOKEN_BYTES)
            return self.read_key

    def destroy(self) -> None:
        """Overwrite and clear all mailbox state, then mark as destroyed."""
        with self.lock:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Scrub sensitive message data and reject late writes on stale refs.
        mailbox.destroy()
        return True
