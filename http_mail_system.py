...
class HttpMailbox:
    ...
    def get_messages(self, read_key: str) -> Optional[List[Dict]]:
        ...
        result = self.query_messages(read_key)
        if result is None:
            return None
    ...
    def query_messages(
        ...
        """Return filtered/paginated messages if read_key matches, else None."""
        ...
        if sender:
            ...
            filtered = [m for m in filtered if m.sender_handle.lower() == sender_lower]

        if subject_contains:
            subject_lower = subject_contains.lower()
            filtered = [m for m in filtered if subject_lower in m.subject.lower()]
        ...
        return {
            "messages": [m.to_dict() for m in page],
            "total": total,
        }
