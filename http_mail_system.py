...
# Maximum number of retained messages per mailbox
...
class MailboxDestroyedError(RuntimeError):
    """Raised when writing to a destroyed mailbox."""
...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> str:
        ...
        with self.lock:
            ...
            if len(self.messages) >= MAX_MESSAGES_PER_MAILBOX:
                raise MailboxFullError("Mailbox capacity reached")
    ...
    def destroy(self) -> None:
        """Overwrite all message material and mark mailbox as destroyed."""
        with self.lock:
            for msg in self.messages:
                msg.overwrite()
            self.messages.clear()
