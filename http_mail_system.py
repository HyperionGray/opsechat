...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Mark mailbox destroyed and securely wipe all in-memory messages."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Wipe data and mark the mailbox destroyed so stale references cannot
        # continue to append/read content.
        mailbox.destroy()
