...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        Concurrency notes:
        - We authenticate and atomically mark mailbox as destroyed while holding
          both storage lock and mailbox lock.
        - We remove the mailbox from the global store before releasing locks.
        - Writers then refuse new sends (`destroyed=True`) and all messages are
          overwritten in-memory before removal.
        """
        with self._lock:
            ...
            # Take mailbox lock while still holding storage lock to avoid races
            # with concurrent writers that already hold a mailbox reference.
            with mailbox.lock:
