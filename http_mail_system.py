...
class HttpMailbox:
    ...
    def get_status(self, read_key: str) -> Optional[Dict]:
        """Return mailbox metadata if read_key is valid, else None."""
        if not secrets.compare_digest(read_key, self.read_key):
            return None
        return {
    ...
    def destroy(self) -> None:
        """Securely wipe mailbox content and mark mailbox as destroyed."""
        with self.lock:
            for msg in self.messages:
                msg.overwrite()
            self.messages.clear()
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Wipe content under mailbox lock and mark mailbox as destroyed.
        mailbox.destroy()
