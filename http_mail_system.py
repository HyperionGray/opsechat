...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Permanently destroy this mailbox and scrub message content."""
        with self.lock:
            if self.destroyed:
                return
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then destroy/scrub mailbox content using the per-mailbox lock.
        """
        ...
        # Now that the mailbox is no longer globally reachable, destroy
        # in-memory content in a thread-safe way.
        mailbox.destroy()
