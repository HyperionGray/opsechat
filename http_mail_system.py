...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Permanently destroy mailbox contents and block future writes."""
        with self.lock:
            if self.destroyed:
                return
            self.destroyed = True
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Destroying the mailbox after removal from global storage protects
        # against stale object references continuing to accept messages.
        mailbox.destroy()
