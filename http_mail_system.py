...
class HttpMailbox:
    ...
    def get_messages(
        ...
        sender_filter = sender.strip().lower()
        text_query = query.strip().lower()
        if sender_filter:
            messages = [m for m in messages if m.sender_handle.lower() == sender_filter]
        if text_query:
            messages = [
                m for m in messages
                if (
                    text_query in m.subject.lower()
                    or text_query in m.body.lower()
                    or text_query in m.sender_handle.lower()
                )
            ]
