...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> Optional[str]:
        """Add a message and return new message ID, or None if mailbox was destroyed."""
        with self.lock:
            if self.destroyed:
                return None
    ...
    def rotate_read_key(self, current_read_key: str) -> Optional[str]:
        """Rotate the mailbox read key after authenticating with the current key."""
        with self.lock:
            if self.destroyed:
                return None
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        if lock is not None:
            with lock:
                # Mark first so concurrent writers immediately refuse sends.
    ...
    def rotate_mailbox_read_key(self, address: str, current_read_key: str) -> Optional[str]:
        """Rotate read key for a mailbox after current key authentication."""
