...
class HttpMailbox:
    ...
    def get_messages(self,
                     ...
                     *,
                     ...
                     newest_first: bool = True) -> Optional[List[Dict]]:
        ...
        sender_term = sender.strip().casefold()
        with self.lock:
            if self.destroyed:
                return []
            messages = list(reversed(self.messages)) if newest_first else list(self.messages)

        if search_term:
            messages = [
                m for m in messages
                if search_term in m.subject.casefold()
                or search_term in m.body.casefold()
                or search_term in m.sender_handle.casefold()
            ]

        if sender_term:
            messages = [m for m in messages if sender_term in m.sender_handle.casefold()]
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # overwrite and clear messages under the per-mailbox lock.
        with mailbox.lock:
