...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Irreversibly destroy mailbox contents and block future writes."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Now that the mailbox is no longer globally reachable, destroy all
        # in-memory message content and block future writes on any stale refs.
        mailbox.destroy()
