...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> str:
        """Add a message and return the new message ID.

        Raises:
            MailboxDestroyedError: mailbox has already been destroyed.
        """
        with self.lock:
            if self.destroyed:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Marking destroyed while holding the lock prevents new writers.
        with mailbox.lock:
    ...
    def rotate_read_key(self, address: str, read_key: str) -> Optional[str]:
        """Rotate read key and return the new key if authenticated."""
        with self._lock:
            mailbox = self._mailboxes.get(address)
        ...
        with mailbox.lock:
            ...
            if not secrets.compare_digest(read_key, mailbox.read_key):
                return None
            mailbox.read_key = _generate_id(24)
            return mailbox.read_key
