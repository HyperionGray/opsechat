...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Securely wipe mailbox content and mark it as destroyed."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then securely wipe and mark it destroyed under the per-mailbox lock
          so stale references can no longer accept writes.
        """
        ...
        # Once globally unreachable, wipe mailbox memory and hard-disable writes.
        mailbox.destroy()
        return True
