...
# Per-mailbox message cap (limits unbounded memory growth)
...
class MailboxDestroyedError(RuntimeError):
    """Raised when writing to a mailbox that has already been destroyed."""


class MailboxCapacityError(RuntimeError):
    """Raised when a mailbox is at message capacity."""
...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> str:
        ...
        with self.lock:
            ...
            self._expire_old_messages_locked()
            if len(self.messages) >= MAX_MESSAGES_PER_MAILBOX:
                raise MailboxCapacityError("Mailbox capacity reached")
    ...
    def get_metadata(self, read_key: str) -> Optional[Dict]:
        ...
        with self.lock:
            ...
            return {
                ...
                "expiry_hours": MAIL_EXPIRY_HOURS,
    ...
    def _expire_old_messages_locked(self) -> None:
        """Remove expired messages. Caller must hold self.lock."""
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        This lifecycle now relies on HttpMailbox using a writer lock and
        enforcing a `destroyed` flag in add_message.
        """
