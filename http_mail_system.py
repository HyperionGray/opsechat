...
class HttpMailbox:
    ...
    def rotate_read_key(self, current_read_key: str) -> Optional[str]:
        """Rotate mailbox read key after verifying current key."""
        with self.lock:
            if self.destroyed:
                return None
    ...
    def destroy(self) -> None:
        """Destroy mailbox contents and block future reads/writes."""
        with self.lock:
            if self.destroyed:
                return
            self.destroyed = True
...
class HttpMailStorage:
    ...
    def rotate_mailbox_read_key(self, address: str, read_key: str) -> Optional[str]:
        """Rotate a mailbox read key after authentication."""
        with self._lock:
            mailbox = self._mailboxes.get(address)
        if mailbox is None:
            return None
