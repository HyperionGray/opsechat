...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> Optional[str]:
        """Add a message unless the mailbox has been destroyed.

        Returns the new message ID on success, or None if writes are refused.
        """
        with self.lock:
            if self.destroyed:
                return None
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then mark it destroyed and overwrite/clear messages under the
          per-mailbox lock to avoid races with concurrent send/add operations.
        """
        ...
        # Now that the mailbox is no longer globally reachable, safely mark it
        # as destroyed and scrub all messages under the mailbox lock.
        with mailbox.lock:
