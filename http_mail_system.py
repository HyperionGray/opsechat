...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> Optional[str]:
        """Add a message and return its ID, or None if mailbox was destroyed."""
        with self.lock:
            if self.destroyed:
                return None
    ...
    def destroy(self) -> bool:
        """Overwrite and clear mailbox messages, marking mailbox as destroyed."""
        with self.lock:
            if self.destroyed:
                return False
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then destroy under the per-mailbox lock to avoid races with
          concurrent send/add operations holding stale mailbox references.
        """
        ...
        # Now that mailbox is no longer globally reachable, mark as destroyed
        # so stale mailbox references cannot accept new writes.
        mailbox.destroy()
