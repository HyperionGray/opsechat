...
class HttpMailbox:
    ...
    def rotate_read_key(self, current_read_key: str) -> Optional[str]:
        """Rotate the mailbox read key after authenticating with current key."""
        with self.lock:
            if self.destroyed:
                return None
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        with self._lock:
            ...
            # Lock ordering: storage lock -> mailbox lock. This prevents:
            # 1) New lookups while deleting, and
            # 2) Concurrent writers mutating mailbox.messages during wipe.
            lock = getattr(mailbox, "lock", None)
            if lock is not None:
                with lock:
                    # Re-check key under mailbox lock to avoid key-rotation races.
    ...
    def rotate_mailbox_key(self, address: str, current_read_key: str) -> Optional[str]:
        """Rotate mailbox read key for an existing mailbox."""
