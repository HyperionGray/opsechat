...
class HttpMailbox:
    ...
    def destroy(self) -> None:
        """Scrub all message data and mark mailbox as permanently destroyed."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        - We then atomically destroy/scrub the mailbox instance to prevent
          stale references from accepting writes.
        """
        ...
        # Scrub in-memory content and reject future writes through any stale refs.
        mailbox.destroy()
