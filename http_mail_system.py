...
class HttpMailbox:
    ...
    def add_message(self, subject: str, body: str, sender_handle: str) -> Optional[str]:
        """Add a message and return message ID, or None if mailbox is destroyed."""
        with self.lock:
            if self.destroyed:
                return None
    ...
    def get_messages(self, read_key: str) -> Optional[List[Dict]]:
        ...
        with self.lock:
            if self.destroyed:
                return None
            key_valid = secrets.compare_digest(read_key, self.read_key)
    ...
    def rotate_read_key(self, read_key: str) -> Optional[str]:
        """Rotate read key and return the new key after successful authentication."""
        with self.lock:
            if self.destroyed:
                return None
            if not secrets.compare_digest(read_key, self.read_key):
                return None
            self.read_key = _generate_id(24)
            return self.read_key

    def destroy(self) -> None:
        """Destroy mailbox content in-memory and block any further writes."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
                msg.overwrite()
            self.messages.clear()
            self.read_key = "X" * len(self.read_key)
            self.destroyed = True
...
class HttpMailStorage:
    ...
    def rotate_read_key(self, address: str, read_key: str) -> Optional[str]:
        """Rotate mailbox read key and return the new key."""
        with self._lock:
            mailbox = self._mailboxes.get(address)
        if mailbox is None:
            return None
        return mailbox.rotate_read_key(read_key)
