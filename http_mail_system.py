...
class HttpMailbox:
    ...
    def mark_destroyed(self) -> None:
        """Wipe messages and mark mailbox as permanently destroyed."""
        with self.lock:
            if self.destroyed:
                return
            for msg in self.messages:
...
class HttpMailStorage:
    ...
    def delete_mailbox(self, address: str, read_key: str) -> bool:
        ...
        # Mark destroyed so stale references refuse new writes and all
        # in-memory message content is scrubbed before release.
