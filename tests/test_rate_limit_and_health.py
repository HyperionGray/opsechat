...
def test_health_endpoint_exposes_runtime_counters():
    client = _test_app.test_client()
    response = client.get("/health")
    data = response.get_json()
...
def test_health_ready_endpoint_reports_readiness_contract():
    client = _test_app.test_client()
    ...
    assert data["checks"]["memory_usage"] in {"ok", "high"}
    assert "runtime" in data
    assert isinstance(data["runtime"]["active_rooms"], int)
