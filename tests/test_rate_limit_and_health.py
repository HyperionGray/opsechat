...
def _clear_runtime_state():
    """Helper: clear global in-memory state shared across tests."""
    with rooms_lock:
        chat_rooms.clear()
    with dm_lock:
...
def test_health_endpoint_returns_json_with_required_fields():
    ...
    assert "simple_chat_rate_limits" in data
...
def test_health_endpoint_reflects_created_room():
    _clear_runtime_state()
    client = _test_app.test_client()
    ...
    assert health["active_rate_limit_sessions"] >= 1
