...
try:
    ...
    class MockBurnerManager:
        ...
        def generate_burner_email(self, user_id):
            ...
            burner_email = f"test{id_generator(8)}@example.com"
            now = datetime.datetime.now()
            self.burner_addresses[burner_email] = {
                "user_id": user_id,
                "created_at": now,
        ...
        def get_user_burners(self, user_id):
            ...
            for email in self.user_burners.get(user_id, []):
                if email in self.burner_addresses:
                    burners.append({"email": email, **self.burner_addresses[email]})
            return burners
        ...
        def expire_burner(self, email):
            ...
            if user_id in self.user_burners:
                self.user_burners[user_id] = [
                    e for e in self.user_burners[user_id] if e != email
                ]
            return True
