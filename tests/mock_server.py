...
try:
    ...
    class MockBurnerManager:
        ...
        def cleanup_expired(self):
            # Mock server does not track burner TTL; keep API parity.
            return 0

        def generate_burner_email(self, user_id):
            burner = f"test-{id_generator(10)}@example.com"
            self.user_burners.setdefault(user_id, []).append(burner)
            self.burner_owner[burner] = user_id
            return burner
        ...
        def get_user_for_burner(self, email):
            return self.burner_owner.get(email)

        def expire_burner(self, email):
            owner = self.burner_owner.pop(email, None)
            if owner is None:
                return False
            if owner in self.user_burners:
                self.user_burners[owner] = [
                    burner for burner in self.user_burners[owner] if burner != email
                ]
            return True
