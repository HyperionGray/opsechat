...
try:
    ...
    class MockBurnerManager:
        def __init__(self):
            ...
            self._user_by_burner = {}

        def cleanup_expired(self):
            return 0

        def generate_burner_email(self, user_id):
            burner_email = f"test{user_id}-{id_generator(size=8)}@example.com"
            self._burners_by_user.setdefault(user_id, []).append(burner_email)
            self._user_by_burner[burner_email] = user_id
            return burner_email
        ...
        def get_user_for_burner(self, email):
            return self._user_by_burner.get(email)

        def expire_burner(self, email):
            owner = self._user_by_burner.pop(email, None)
            if owner is None:
                return False
            self._burners_by_user[owner] = [
                burner for burner in self._burners_by_user.get(owner, [])
                if burner != email
            ]
            return True
