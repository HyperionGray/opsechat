...
class TestBurnerEmailManager:
    ...
    def test_cleanup_expired_removes_user_index_references(self):
        """Expired burners should be removed from both indexes."""
        manager = BurnerEmailManager()
        expired_email = manager.generate_burner_email("user1")
        ...
        assert "user1" not in manager.user_burners or expired_email not in manager.user_burners["user1"]

    def test_get_user_stats_returns_burner_and_rate_limit_data(self):
        """Burner stats API should include inventory and send-limit details."""
        manager = BurnerEmailManager()
        ...
        assert stats["active_burners_count"] == 2
        assert stats["expiring_soon_count"] >= 0
        assert stats["total_time_remaining_seconds"] > 0
        assert isinstance(stats["total_time_remaining_str"], str)
        assert "send_limit" in stats
        assert stats["send_limit"]["sends_used"] == 1
