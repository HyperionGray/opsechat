...
class TestDMRoutes:
    ...
    def test_view_dm_is_burned_after_first_read(self, client):
        room_id = client.post("/chat/create").get_json()["room_id"]
        dm_id = client.post(
            "/chat/dm/send",
            json={"room_id": room_id, "message": "one-time invite"},
            content_type="application/json",
        ).get_json()["dm_id"]
...
class TestBugFixes:
    ...
    def test_chat_room_has_no_inline_script(self, client):
        ...
        assert "simple-chat-room.css" in body
        assert "addEventListener" not in body
        assert "onclick=" not in body
        assert "<style>" not in body
        assert "style=" not in body
