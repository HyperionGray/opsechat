...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using form/json with address field
...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(address: str):
        ...
        try:
            ...
            return _render_mail_page(error="Mailbox is unavailable",
    ...
    # Supports both explicit mailbox URL and a no-JS fallback endpoint.
    # ------------------------------------------------------------------
    ...
    def http_mail_send_generic(url_addition):
        ...
        if not address:
            ...
            return _render_mail_page(error="Mailbox address is required", compose_address=""), 400
    ...
    def http_mail_rotate_key(url_addition, address):
        ...
        new_read_key = http_mail_storage.rotate_mailbox_key(address, read_key)
        if new_read_key is None:
            if _wants_json():
                return jsonify({"error": "Invalid read key"}), 403
            return _render_mail_page(error="Invalid read key - cannot rotate",
                                     inbox_address=address), 403
