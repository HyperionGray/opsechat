...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - No-JS send helper (address in form field)
  POST /<path>/mail/<address>/send         - Send a message to a mailbox (no auth)
  GET  /<path>/mail/<address>/inbox        - Read inbox (requires ?key=<read_key>, supports filters)
...
def register_http_mail_routes(app):
    ...
    def _send_to_address(address: str):
        ...
        if msg_id is None:
            # Mailbox may have been concurrently destroyed after lookup.
            if request.is_json:
    ...
    def http_mail_inbox(url_addition, address):
        ...
        limit = _parse_limit_arg(limit_raw)
        if limit_raw and limit is None:
            if _accepts_json():
                return jsonify({"error": "Invalid limit; expected integer"}), 400
            return render_template("http_mail.html",
                                   path=app.config["path"],
                                   hostname=app.config.get("hostname", ""),
                                   max_message_length=MAX_MAIL_MESSAGE_LENGTH,
                                   error="Invalid limit — use a number between 1 and 200"), 400
