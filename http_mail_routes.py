...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using address in payload (no-JS form support)
...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(url_addition: str, address: str):
        """Send a message to an existing mailbox, returning Flask response."""
    ...
    def http_mail_send_from_form(url_addition):
        """Send using mailbox address in payload (non-JS form fallback)."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        if request.is_json:
        ...
        if not address:
            ...
            return render_template("http_mail.html",
                                   ...
                                   hostname=app.config.get("hostname", ""),
                                   max_message_length=MAX_MAIL_MESSAGE_LENGTH,
                                   error="Mailbox address is required to send",
                                   compose_address=""), 400
