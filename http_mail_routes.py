...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Generic send endpoint (address in payload)
...
def register_http_mail_routes(app):
    ...
    def _wants_json_response() -> bool:
        """Detect whether caller expects JSON response semantics."""
        if request.is_json:
            return True
    ...
    def _extract_send_payload():
        """Read message fields from JSON or form payload."""
        if request.is_json:
    ...
    def _handle_send_to_address(url_addition: str, address: str):
        """Shared send implementation for both send routes."""
    ...
    def http_mail_send_generic(url_addition):
        """No-JS form endpoint: accepts address in payload and sends message."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
