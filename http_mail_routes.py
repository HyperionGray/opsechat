...
def register_http_mail_routes(app):
    ...
    def _send_message_response(url_addition: str, address: str):
        """Shared send-message handler for path and generic form routes."""
    ...
    def http_mail_send(url_addition, address):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        return _send_message_response(url_addition, address)

    @app.route('/<string:url_addition>/mail/send', methods=["POST"])
    def http_mail_send_generic(url_addition):
        """No-JS fallback send route that accepts address in form/json body."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
