...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using address from payload (no JS fallback)
...
def register_http_mail_routes(app):
    ...
    def _extract_send_fields():
        """Read send payload from JSON or form and normalize fields."""
        if request.is_json:
    ...
    def _send_to_mailbox(address: str):
        ...
        _, subject, body, sender = _extract_send_fields()
    ...
    def http_mail_send_nojs(url_addition):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        address, _, _, _ = _extract_send_fields()
        if not address:
            if request.is_json:
