...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send (address provided in body/form)
...
def register_http_mail_routes(app):
    ...
    def _render_mail_page(status_code: int = 200, **context):
        """Render the mail UI with standard context."""
    ...
    def _send_to_address(address: str):
        """Send message payload to a mailbox address, supporting JSON and form."""
        mailbox = http_mail_storage.get_mailbox(address)
        if mailbox is None:
            if request.is_json:
                return jsonify({"error": "Mailbox not found"}), 404
            return _render_mail_page(404, error="Mailbox not found", compose_address=address)
        ...
        if not body:
            ...
            return _render_mail_page(400, error="Message body is required", compose_address=address)
    ...
    def http_mail_send_form(url_addition):
        ...
        if not address:
            ...
            return _render_mail_page(400, error="Recipient mailbox address is required")
