...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send with address in request body (no auth)
...
def register_http_mail_routes(app):
    ...
    def _extract_message_fields():
        """Extract message payload from JSON or form inputs."""
        if request.is_json:
    ...
    def _send_to_mailbox(url_addition: str, address: str):
        """Send a message to an address and emit JSON or HTML response."""
    ...
    def http_mail_send_with_form_address(url_addition):
        """No-JS friendly send endpoint: mailbox address is supplied in form body."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
