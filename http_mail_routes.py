...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Fallback send endpoint (address in body)
...
def register_http_mail_routes(app):
    ...
    def _sanitize_address(address: str) -> str:
        """Normalize mailbox address token."""
        return re.sub(r'[^a-zA-Z0-9_-]', '', (address or '').strip())[:128]

    def _send_to_address(address: str):
        """Handle shared send logic for both route variants."""
        ...
        if msg_id is None:
            # Mailbox was destroyed during a concurrent request.
            if request.is_json:
                return jsonify({"error": "Mailbox no longer accepts messages"}), 409
            return render_template("http_mail.html",
                                   path=app.config["path"],
                                   hostname=app.config.get("hostname", ""),
                                   max_message_length=MAX_MAIL_MESSAGE_LENGTH,
                                   error="Mailbox no longer accepts messages"), 409
    ...
    def http_mail_send(url_addition, address):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        return _send_to_address(_sanitize_address(address))

    @app.route('/<string:url_addition>/mail/send', methods=["POST"])
    def http_mail_send_fallback(url_addition):
        """No-JS compose fallback route using posted mailbox address."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()

        if request.is_json:
            address = _sanitize_address((request.get_json() or {}).get("address", ""))
        else:
            address = _sanitize_address(request.form.get("_address_override", ""))
        ...
        return _send_to_address(address)
