...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(address: str):
        """Common send flow used by both /mail/<address>/send and /mail/send."""
        ...
        if msg_id is None:
            # Mailbox was destroyed after lookup but before append.
            message = "Mailbox was destroyed. Ask recipient for a new mailbox address."
            if request.is_json:
    ...
    def http_mail_send_nojs(url_addition):
        """No-JS compose fallback endpoint; address comes from payload."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()

        if request.is_json:
            address = (request.get_json() or {}).get("address", "")
        else:
