...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using address from payload/form
...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(address: str, subject: str, body: str, sender: str):
        if not address:
            if request.is_json:
                return jsonify({"error": "Mailbox address is required"}), 400
            return _render_mail_page(error="Mailbox address is required"), 400
    ...
    # ------------------------------------------------------------------
    # Send a message (mailbox address is in payload/form body)
    # ------------------------------------------------------------------
    ...
    def http_mail_send_generic(url_addition):
        ...
        return _send_to_mailbox(address, subject, body, sender)
