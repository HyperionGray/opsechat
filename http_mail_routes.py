...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using form-provided address
...
def register_http_mail_routes(app):
    ...
    def _render_mail_page(**kwargs):
        """Render HTTP mail UI with common context fields."""
    ...
    def _handle_send_to_address(url_addition: str, address: str):
        """Handle message send flow for both URL-address and form-address routes."""
        ...
        if msg_id is None:
            # Race-safe behavior: mailbox can be destroyed between lookup and send.
            if request.is_json:
    ...
    # ------------------------------------------------------------------
    # Send a message using address provided in form body (no JavaScript)
    # ------------------------------------------------------------------
    ...
    def http_mail_send_form(url_addition):
        ...
        return _handle_send_to_address(url_addition, address)
