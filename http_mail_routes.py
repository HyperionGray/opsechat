...
Routes registered under /<path>/mail/:
  ...
  GET  /<path>/mail/<address>/status       - Mailbox stats (requires ?key=<read_key>)
...
from http_mail_system import http_mail_storage, MAX_MAIL_MESSAGE_LENGTH, MAIL_EXPIRY_HOURS
...
def register_http_mail_routes(app):
    ...
    # ------------------------------------------------------------------
    # Mailbox status (requires read_key) - JSON API
    # ------------------------------------------------------------------
    ...
    def http_mail_status(url_addition, address):
        ...
        return jsonify({
            ...
            "message_ttl_hours": MAIL_EXPIRY_HOURS,
            "inbox_url": f"/{url_addition}/mail/{address}/inbox",
        })
