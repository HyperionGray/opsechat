...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Non-JS send fallback (address in form data)
...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(mailbox, address: str):
        """Shared send logic for addressed and fallback compose endpoints."""
        ...
        if msg_id is None:
            # Mailbox existed but was destroyed while this request was in flight.
            if _wants_json_response():
    ...
    def http_mail_send(url_addition, address):
        ...
        return _send_to_mailbox(mailbox, address)

    @app.route('/<string:url_addition>/mail/send', methods=["POST"])
    def http_mail_send_fallback(url_addition):
        """Non-JS compose fallback where mailbox address is part of form data."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
    ...
    def http_mail_rotate_key(url_addition, address):
        """Rotate mailbox read key using the current read key."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
