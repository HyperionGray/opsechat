...
def register_http_mail_routes(app):
    ...
    def _sanitize_address(address: str) -> str:
        """Keep mailbox address URL-safe and compact."""
        return re.sub(r'[^A-Za-z0-9_-]', '', address)[:128]

    def _wants_json_response() -> bool:
        """Return True when the caller expects JSON."""
        if request.is_json:
            return True
    ...
    def _render_http_mail(**kwargs):
        """Render HTTP mail template with common context."""
    ...
    def _send_message_to_mailbox(address: str, subject: str, body: str, sender: str):
        """Shared send implementation for both direct and address routes."""
        mailbox = http_mail_storage.get_mailbox(address)
        if mailbox is None:
            if _wants_json_response():
                return jsonify({"error": "Mailbox not found"}), 404
            return _render_http_mail(error="Mailbox not found", compose_address=address), 404
    ...
    def http_mail_send(url_addition, address):
        ...
        safe_address = _sanitize_address(address.strip())
        if request.is_json:
        ...
        return _send_message_to_mailbox(
    ...
    def http_mail_send_direct(url_addition):
        """Direct compose endpoint that accepts mailbox address in payload."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        if request.is_json:
            data = request.get_json(silent=True) or {}
            address = _sanitize_address(str(data.get("address", "")).strip())
            subject = str(data.get("subject", "")).strip()
            body = str(data.get("body", "")).strip()
            sender = str(data.get("sender", "anonymous")).strip()
        else:
            address = _sanitize_address(request.form.get("address", "").strip())
            subject = request.form.get("subject", "").strip()
            body = request.form.get("body", "").strip()
            sender = request.form.get("sender", "anonymous").strip()
        ...
        if not address:
            ...
            return _render_http_mail(error="Mailbox address is required", compose_address=""), 400
    ...
    def http_mail_inbox_lookup(url_addition):
        """Read inbox by address/key query params (HTML form fallback)."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()

        address = _sanitize_address(request.args.get("address", "").strip())
        read_key = request.args.get("key", "").strip()

        if not address or not read_key:
            if _wants_json_response():
                return jsonify({"error": "Mailbox address and read key are required"}), 400
            return _render_http_mail(error="Mailbox address and read key are required"), 400
