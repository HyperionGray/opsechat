...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send via form payload (_address_override)
...
def register_http_mail_routes(app):
    ...
    def _extract_message_fields():
        """Extract message fields from JSON or form payload."""
        if request.is_json:
    ...
    def _send_to_mailbox(address: str):
        """Validate and send a message to an existing mailbox."""
    ...
    def http_mail_send_form_fallback(url_addition):
        """Form fallback route that accepts recipient address in payload."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
