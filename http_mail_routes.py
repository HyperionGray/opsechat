...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send message via form address (no-JS fallback)
  POST /<path>/mail/<address>/send         - Send a message to a mailbox (no auth)
  GET  /<path>/mail/<address>/status       - Mailbox metadata (requires ?key=<read_key>)
...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(address: str):
        ...
        if msg_id is None:
            # Mailbox existed at lookup time but was destroyed before write.
            if request.is_json:
                return jsonify({"error": "Mailbox unavailable"}), 410
            return _render_mail(
                error="Mailbox unavailable. Ask recipient for a new mailbox address.",
                compose_address=address,
            ), 410
    ...
    # ------------------------------------------------------------------
    # Send from form with explicit address field (no-JS fallback)
    # ------------------------------------------------------------------
    ...
    def http_mail_send_nojs(url_addition):
        ...
        if not address:
            return _render_mail(error="Recipient mailbox address is required."), 400

        return _send_to_mailbox(address)

    # ------------------------------------------------------------------
    # Mailbox metadata/status (requires read_key)
    # ------------------------------------------------------------------
    ...
    def http_mail_status(url_addition, address):
        ...
        return jsonify({"success": True, "mailbox": status})
