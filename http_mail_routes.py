...
def register_http_mail_routes(app):
    ...
    def _mail_error_response(address: str, message: str, status_code: int):
        if request.is_json:
        ...
        return render_template("http_mail.html",
                               ...
                               error=message,
                               compose_address=address), status_code

    def _send_to_mailbox(address: str):
        mailbox = http_mail_storage.get_mailbox(address)
        if mailbox is None:
            return _mail_error_response(address, "Mailbox not found", 404)
        ...
        if not body:
            return _mail_error_response(address, "Message body is required", 400)
        ...
        if msg_id is None:
            return _mail_error_response(address, "Mailbox is no longer available", 410)
    ...
    def http_mail_send_with_form_address(url_addition):
        """No-JS form fallback: send endpoint using address provided in form/json."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        if request.is_json:
            ...
            address = payload.get("_address_override", "").strip() or payload.get("address", "").strip()
        else:
            address = request.form.get("_address_override", "").strip() or request.form.get("address", "").strip()

        address = re.sub(r'[^A-Za-z0-9_-]', '', address)[:64]
        if not address:
            return _mail_error_response("", "Recipient mailbox address is required", 400)
