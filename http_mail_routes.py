...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send via form/JSON with address in payload
...
def register_http_mail_routes(app):
    ...
    def _wants_json() -> bool:
        ...
        return request.is_json or accept_header.startswith("application/json")

    def _render_mail(**kwargs):
    ...
    def _send_to_mailbox(address: str):
        ...
        if mailbox is None:
            ...
            return _render_mail(error="Mailbox not found", compose_address=address), 404

        _, subject, body, sender = _parse_send_payload()
        ...
        if msg_id is None:
            # Mailbox was destroyed while request was in flight.
            if _wants_json():
        ...
        return _render_mail(success="Message sent.", compose_address=address)
    ...
    def http_mail_create(url_addition):
        ...
        return jsonify({
            ...
            "read_url": f"/{url_addition}/mail/{mailbox.address}/inbox?key={mailbox.read_key}",
        })

    # ------------------------------------------------------------------
    # Send a message via explicit address (form/JSON payload)
    # ------------------------------------------------------------------
    ...
    def http_mail_send_generic(url_addition):
        ...
        address, _, _, _ = _parse_send_payload()
        address = _sanitize(address, 64)
        if not address:
            if _wants_json():
                return jsonify({"error": "Mailbox address is required"}), 400
            return _render_mail(error="Mailbox address is required", compose_address=""), 400
        return _send_to_mailbox(address)

    # ------------------------------------------------------------------
    # Send a message to a mailbox via URL path (no auth required)
    # ------------------------------------------------------------------
    ...
    def http_mail_send(url_addition, address):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        return _send_to_mailbox(_sanitize(address, 64))
    ...
    def http_mail_inbox(url_addition, address):
        ...
        if mailbox is None:
            if _wants_json():
                return jsonify({"error": "Mailbox not found"}), 404
            return _render_mail(error="Mailbox not found"), 404
        ...
        if messages is None:
            ...
            return _render_mail(error="Invalid read key — access denied"), 403
    ...
    def http_mail_destroy(url_addition, address):
        ...
        return _render_mail(success="Mailbox destroyed.")
