...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send message using address in request body (no-JS)
  POST /<path>/mail/<address>/send         - Send a message to a mailbox (no auth)
  GET  /<path>/mail/inbox                  - Read inbox using ?address=...&key=... (no-JS)
...
def register_http_mail_routes(app):
    ...
    def _wants_json() -> bool:
        """Return True when caller explicitly expects JSON."""
        accept = request.headers.get("Accept", "")
        return request.is_json or "application/json" in accept

    def _send_to_mailbox(address: str):
        """Handle message send to a known mailbox address."""
    ...
    def http_mail_send_direct(url_addition):
        """No-JS fallback route: send using address in request body."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        if request.is_json:
            data = request.get_json() or {}
            address = (data.get("address", "") or data.get("_address_override", "")).strip()
        else:
    ...
    def http_mail_inbox_direct(url_addition):
        """No-JS fallback route: read inbox via address/key query parameters."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()

        address = (request.args.get("address", "") or
                   request.args.get("_read_address", "")).strip()
        read_key = (request.args.get("key", "") or
                    request.args.get("_read_key", "")).strip()
        ...
        if not address:
            ...
            return render_template("http_mail.html",
                                   ...
                                   hostname=app.config.get("hostname", ""),
                                   max_message_length=MAX_MAIL_MESSAGE_LENGTH,
                                   error="Mailbox address is required to read inbox."), 400
