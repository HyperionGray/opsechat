...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(address: str):
        """Shared send-message handler used by both send routes."""
        ...
        if msg_id is None:
            if request.is_json:
                return jsonify({"error": "Mailbox is destroyed or unavailable"}), 410
            return render_template("http_mail.html",
                                   path=app.config["path"],
                                   hostname=app.config.get("hostname", ""),
                                   max_message_length=MAX_MAIL_MESSAGE_LENGTH,
                                   error="Mailbox is destroyed or unavailable",
    ...
    def http_mail_send_dynamic(url_addition):
        """Send endpoint for non-JS clients using address in body/json."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
