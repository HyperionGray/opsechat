...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send via no-JS fallback form body
...
def register_http_mail_routes(app):
    ...
    def _extract_address_from_request() -> str:
        """Extract mailbox address from JSON or form for non-JS fallback sends."""
        if request.is_json:
            return (request.get_json() or {}).get("address", "").strip()
        # Legacy and progressive-enhancement form fields.
        return (
    ...
    def _send_to_mailbox(address: str):
        ...
        if msg_id is None:
            ...
            return _render_mail_ui(
                error="Mailbox no longer available",
                compose_address=address,
            ), 410
    ...
    # ------------------------------------------------------------------
    # Send a message (no-JS fallback: address in request body)
    # ------------------------------------------------------------------
    ...
    def http_mail_send_fallback(url_addition):
        ...
        address = _extract_address_from_request()
        if not address:
            if _wants_json_response():
                return jsonify({"error": "Mailbox address is required"}), 400
            return _render_mail_ui(error="Recipient mailbox address is required"), 400
