...
Routes registered under /<path>/mail/:
  ...
  GET  /<path>/mail/<address>/info         - Mailbox metadata (requires ?key=<read_key>)
...
def register_http_mail_routes(app):
    ...
    @app.route('/<string:url_addition>/mail/<string:address>/info', methods=["GET"])
    def http_mail_info(url_addition, address):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        read_key = request.args.get("key", "")
        info = mailbox.get_mailbox_info(read_key)
        if info is None:
            return jsonify({"error": "Invalid read key"}), 403
