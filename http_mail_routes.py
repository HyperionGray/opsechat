...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send a message (form fallback with address field)
...
def register_http_mail_routes(app):
    ...
    def _send_to_mailbox(address: str):
        ...
        if not body:
            ...
            return _render_mail(error="Message body is required",
        ...
        try:
            ...
            # Race-safe behavior when the mailbox was destroyed while this request was in flight.
            if request.is_json:
                return jsonify({"error": "Mailbox no longer available"}), 410
            return _render_mail(error="Mailbox no longer available",
    ...
    # ------------------------------------------------------------------
    # Send a message using form-provided address (non-JS fallback)
    # ------------------------------------------------------------------
    ...
    def http_mail_send_fallback(url_addition):
        ...
        if not address:
            return _render_mail(error="Recipient mailbox address is required",
                                compose_address=""), 400
    ...
    def http_mail_inbox(url_addition, address):
        ...
        return _render_mail(inbox_address=address,
