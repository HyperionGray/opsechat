...
def register_http_mail_routes(app):
    ...
    def _process_send_to_mailbox(url_addition: str, address: str):
        """Handle message submission for a specific mailbox address."""
    ...
    def http_mail_send_fallback(url_addition):
        """No-JS fallback: sender includes target mailbox address in form data."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        return _process_send_to_mailbox(url_addition, address)
    ...
    def http_mail_rotate_key(url_addition, address):
        ...
        if new_key is None:
            ...
            return render_template("http_mail.html",
                                   ...
                                   hostname=app.config.get("hostname", ""),
                                   max_message_length=MAX_MAIL_MESSAGE_LENGTH,
                                   error="Invalid read key — key rotation denied",
                                   inbox_address=address,
