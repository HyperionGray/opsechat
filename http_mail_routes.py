...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using form field-provided mailbox
  ...
  POST /<path>/mail/<address>/rotate-key   - Rotate read_key (requires current key)
...
def register_http_mail_routes(app):
    ...
    def _render_http_mail(status_code=200, **kwargs):
        """Render the HTTP mail template with standard context."""
        context = {
    ...
    def http_mail_send_with_address(url_addition):
        """Form-friendly send endpoint that accepts address in request body."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        if not address:
            ...
            return _render_http_mail(400, error="Mailbox address is required")
    ...
    def http_mail_rotate_key(url_addition, address):
        ...
        if http_mail_storage.get_mailbox(address) is None:
            if _request_prefers_json():
                return jsonify({"error": "Mailbox not found"}), 404
            return _render_http_mail(404, error="Mailbox not found", inbox_address=address)
        ...
        if new_read_key is None:
            if _request_prefers_json():
                return jsonify({"error": "Invalid read key"}), 403
            return _render_http_mail(403, error="Invalid read key - access denied",
                                     inbox_address=address, inbox_read_key=read_key)

        if _request_prefers_json():
            return jsonify({
                "success": True,
                "new_read_key": new_read_key,
                "inbox_url": f"/{url_addition}/mail/{address}/inbox?key={new_read_key}",
            })
    ...
    def http_mail_destroy(url_addition, address):
        ...
        return _render_http_mail(success="Mailbox destroyed.")[0]
