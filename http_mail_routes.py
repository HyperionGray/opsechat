...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Form fallback send (uses _address_override)
...
def register_http_mail_routes(app):
    ...
    # ------------------------------------------------------------------
    # Send fallback route for non-JS form submissions.
    # The HTML compose form posts here and supplies mailbox address in
    # `_address_override`; this keeps compose functional without JavaScript.
    # ------------------------------------------------------------------
    ...
    def http_mail_send_form(url_addition):
        ...
        # Reuse the main send handler so validation/limits stay identical.
