...
def register_http_mail_routes(app):
    ...
    def _send_message_to_mailbox(url_addition: str, address: str):
        """Shared send-message handler for both JSON and non-JS form routes."""
    ...
    def http_mail_send(url_addition, address):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        return _send_message_to_mailbox(url_addition, address)

    @app.route('/<string:url_addition>/mail/send', methods=["POST"])
    def http_mail_send_non_js(url_addition):
        """Non-JS fallback send endpoint that accepts recipient in form field."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
