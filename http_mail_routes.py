...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using payload mailbox address
...
def register_http_mail_routes(app):
    ...
    def _render_mail_template(**context):
    ...
    def _extract_message_input():
        ...
        sender = (data.get("sender", "anonymous") or "anonymous").strip()
        return subject, body, sender

    def _send_message_to_address(address: str):
        mailbox = http_mail_storage.get_mailbox(address)
        if mailbox is None:
            if _wants_json():
                return jsonify({"error": "Mailbox not found"}), 404
            return _render_mail_template(error="Mailbox not found", compose_address=address), 404

        subject, body, sender = _extract_message_input()
        if not body:
            if _wants_json():
                return jsonify({"error": "Message body is required"}), 400
            return _render_mail_template(
        ...
        try:
            ...
            # Mailbox can be destroyed after lookup but before write.
            if _wants_json():
                return jsonify({"error": "Mailbox not found"}), 404
            return _render_mail_template(
        ...
        return _render_mail_template(success="Message sent.", compose_address=address)
    ...
    # ------------------------------------------------------------------
    # Send a message using a payload-provided mailbox address
    # ------------------------------------------------------------------
    ...
    def http_mail_send_generic(url_addition):
        ...
        if request.is_json:
            ...
            address = (request.form.get("_address_override", "") or "").strip()

        if not address:
            if _wants_json():
                return jsonify({"error": "Mailbox address is required"}), 400
            return _render_mail_template(error="Recipient mailbox address is required"), 400

        return _send_message_to_address(address)
    ...
    def http_mail_inbox(url_addition, address):
        ...
        if mailbox is None:
            if _wants_json():
                return jsonify({"error": "Mailbox not found"}), 404
            return _render_mail_template(error="Mailbox not found"), 404
        ...
        if messages is None:
            ...
            return _render_mail_template(error="Invalid read key — access denied"), 403
    ...
    def http_mail_destroy(url_addition, address):
        ...
        return _render_mail_template(success="Mailbox destroyed.")
