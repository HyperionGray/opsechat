...
def register_http_mail_routes(app):
    ...
    def _render_mail_page(**kwargs):
        """Render HTTP mail page with required shared context."""
    ...
    def _send_to_mailbox(address: str):
        """Shared send logic for address-specific and form-based send routes."""
    ...
    def http_mail_send_form(url_addition):
        """No-JS form endpoint: address is provided in _address_override field."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
