...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - No-JS compose fallback (address in form)
...
def register_http_mail_routes(app):
    ...
    def _render_send_error(url_addition: str, address: str, message: str, status_code: int):
    ...
    def http_mail_send(url_addition, address):
        ...
        if mailbox is None:
            ...
            return _render_send_error(url_addition, address, "Mailbox not found", 404)
        ...
        if msg_id is None:
            ...
            return _render_send_error(url_addition, address, "Mailbox has been destroyed", 410)
    ...
    def http_mail_send_via_form(url_addition):
        """No-JS compose fallback route that accepts mailbox address in form body."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        if not address:
            return _render_send_error(url_addition, "", "Recipient mailbox address is required", 400)
        ...
        if not body:
            return _render_send_error(url_addition, address, "Message body is required", 400)
