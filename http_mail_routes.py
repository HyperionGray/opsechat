...
def register_http_mail_routes(app):
    ...
    def _valid_mailbox_address(address: str) -> bool:
        """Validate the public mailbox address format."""
        return bool(re.fullmatch(r"[A-Za-z0-9_-]{8,128}", address))

    def _extract_send_payload():
        """Get subject/body/sender from JSON or form payload."""
        if request.is_json:
    ...
    def _send_to_mailbox(address: str):
        """Shared sender for both /mail/send and /mail/<address>/send."""
        if not _valid_mailbox_address(address):
            if request.is_json:
        ...
        if msg_id is None:
            ...
            return render_template("http_mail.html",
                                   ...
                                   hostname=app.config.get("hostname", ""),
                                   max_message_length=MAX_MAIL_MESSAGE_LENGTH,
                                   error="Mailbox is unavailable (it may have been destroyed).",
    ...
    def http_mail_send_nojs(url_addition):
        """No-JS fallback: address comes from form/json body instead of URL."""
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
