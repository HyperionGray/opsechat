...
def register_http_mail_routes(app):
    ...
    def _cleanup_stale_mailboxes():
        # Opportunistic cleanup to keep long-running instances tidy.
        http_mail_storage.cleanup_empty_old_mailboxes()
    ...
    def http_mail_index(url_addition):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        _cleanup_stale_mailboxes()
        return render_template("http_mail.html", **_base_context())
    ...
    def http_mail_send(url_addition, address):
        ...
        if not body:
            ...
            return render_template(
                "http_mail.html",
                **_base_context(error="Message body is required",
        ...
        return render_template(
            "http_mail.html",
            **_base_context(success="Message sent.", compose_address=address),
        )
    ...
    def http_mail_inbox(url_addition, address):
        ...
        if mailbox is None:
            ...
            return render_template(
                "http_mail.html",
                **_base_context(error="Mailbox not found"),
            ), 404
        ...
        if messages is None:
            ...
            return render_template(
                "http_mail.html",
                **_base_context(error="Invalid read key - access denied"),
            ), 403
        ...
        return render_template(
            "http_mail.html",
            **_base_context(inbox_address=address, inbox_read_key=read_key, messages=messages),
        )
    ...
    @app.route('/<string:url_addition>/mail/<string:address>/rotate-key',
               methods=["POST"])
    def http_mail_rotate_key(url_addition, address):
        ...
        new_key = http_mail_storage.rotate_mailbox_key(address, read_key)
        if not new_key:
        ...
        mailbox = http_mail_storage.get_mailbox(address)
        messages = mailbox.get_messages(new_key) if mailbox else []
        return render_template(
            "http_mail.html",
            **_base_context(
    ...
    def http_mail_destroy(url_addition, address):
        ...
        return render_template("http_mail.html",
                               **_base_context(success="Mailbox destroyed."))
