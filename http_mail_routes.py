...
def register_http_mail_routes(app):
    ...
    def http_mail_send(url_addition, address):
        ...
        try:
            ...
            if request.is_json:
                return jsonify({"error": error}), 410
            return _render_http_mail(error=error, compose_address=address), 410
        except MailboxFullError:
            error = f"Mailbox is full (limit: {MAX_MESSAGES_PER_MAILBOX} messages)"
            if request.is_json:
                return jsonify({"error": error}), 429
            return _render_http_mail(error=error, compose_address=address), 429
