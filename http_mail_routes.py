...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - No-JS form fallback (address in form body)
...
def register_http_mail_routes(app):
    ...
    def _send_message_to_address(url_addition: str, address: str):
        mailbox = http_mail_storage.get_mailbox(address)
        if mailbox is None:
            return jsonify({"error": "Mailbox not found"}), 404

        subject, body, sender = _extract_message_fields()
        ...
        if msg_id is None:
            # Mailbox was concurrently destroyed between lookup and send.
    ...
    def http_mail_send_form_fallback(url_addition):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        return _send_message_to_address(url_addition, address)
    ...
    # ------------------------------------------------------------------
    # Rotate mailbox read key (requires current read_key in POST body)
    # ------------------------------------------------------------------
    ...
    def http_mail_rotate_key(url_addition, address):
        ...
        new_key = mailbox.rotate_read_key(read_key)
        if new_key is None:
            return jsonify({"error": "Invalid read key or mailbox unavailable"}), 403
        ...
        return render_template("http_mail.html",
                               ...
                               messages=mailbox.get_messages(new_key) or [])
