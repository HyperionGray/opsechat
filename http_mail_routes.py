...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send to mailbox (address in payload)
...
def register_http_mail_routes(app):
    ...
    def _extract_send_fields():
        """Read send payload from JSON or form."""
        if request.is_json:
    ...
    def _send_to_mailbox(address: str):
        """Shared send logic for URL-addressed and payload-addressed sends."""
    ...
    # ------------------------------------------------------------------
    # Send a message with address in payload (no authentication required)
    # ------------------------------------------------------------------

    @app.route('/<string:url_addition>/mail/send', methods=["POST"])
    def http_mail_send_by_payload(url_addition):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
        ...
        if request.is_json:
            ...
            address = (request.form.get("address")
                       or request.form.get("_address_override")
                       or "").strip()
