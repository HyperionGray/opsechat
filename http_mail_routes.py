...
Routes registered under /<path>/mail/:
  ...
  GET  /<path>/mail/<address>/status       - Read inbox metadata only (requires ?key=<read_key>)
...
def register_http_mail_routes(app):
    ...
    # ------------------------------------------------------------------
    # Read mailbox status metadata (requires read_key)
    # ------------------------------------------------------------------
    ...
    def http_mail_status(url_addition, address):
        ...
        status["expiry_hours"] = MAIL_EXPIRY_HOURS
