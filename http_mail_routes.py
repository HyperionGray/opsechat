...
Routes registered under /<path>/mail/:
  ...
  POST /<path>/mail/send                   - Send using address in body/form
...
def register_http_mail_routes(app):
    ...
    def _extract_send_fields():
        """Read sender/subject/body from JSON or form payload."""
        if request.is_json:
    ...
    def _send_message(address: str):
        """Shared send handler used by address and fallback send routes."""
    ...
    def http_mail_send_without_path_address(url_addition):
        if url_addition != app.config["path"]:
            return ('', 404)
        _ensure_session()
