...
def register_email_routes(app, id_generator, get_random_color):
    ...
    def _ensure_session():
        """Ensure session identity is initialized."""
    ...
    def email_config(url_addition):
        ...
        if request.method == "POST":
            ...
            if action == "configure_smtp":
                ...
                smtp_port_raw = request.form.get("smtp_port", "587")
                smtp_username = request.form.get("smtp_username", "").strip()
                smtp_password = request.form.get("smtp_password", "").strip()
                use_tls = request.form.get("use_tls", "").lower() in {"on", "true", "1", "yes"}
                try:
                    ...
                    if configured:
                        ...
                        message = {"type": "error", "text": "SMTP connection test failed"}
                except (TypeError, ValueError):
                    message = {"type": "error", "text": "Invalid SMTP port"}

            elif action == "configure_imap":
                imap_server = request.form.get("imap_server", "").strip()
                imap_port_raw = request.form.get("imap_port", "993")
                imap_username = request.form.get("imap_username", "").strip()
                imap_password = request.form.get("imap_password", "").strip()
                use_ssl = request.form.get("use_ssl", "").lower() in {"on", "true", "1", "yes"}
                try:
                    ...
                    if configured:
                        ...
                        message = {"type": "error", "text": "IMAP connection test failed"}
                except (TypeError, ValueError):
                    message = {"type": "error", "text": "Invalid IMAP port"}
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        message = {
                            "type": "error",
                            "text": result.get("message", "Domain API configuration failed"),
                        }
                except ValueError:
                    message = {"type": "error", "text": "Invalid monthly budget"}
            else:
                # Backwards compatibility with older posted forms.
                config_data = {
    ...
    def email_domain_rotate(url_addition):
        """Rotate to a newly purchased burner email domain."""
        if url_addition != app.config["path"]:
            return ('', 404)
        ...
        if new_domain:
            session["_email_config_message"] = {
                "type": "success",
                "text": f"Successfully rotated active domain to {new_domain}",
            }
        else:
            session["_email_config_message"] = {
                "type": "error",
